    ...
    <el-card>
      ...
      <el-table :data="alerts" v-loading="loading" stripe>
        ...
        <el-table-column label="类型" width="120">
          <template #default="{ row }">
            <el-tag size="small">{{ { battery_low: '电量低', task_timeout: '任务超时', comm_lost: '通信中断', fault: '故障', collision: '碰撞' }[row.alert_type] || row.alert_type }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="状态" width="90">
          <template #default="{ row }">
            <el-tag :type="row.status === 'unack' ? 'danger' : row.status === 'acknowledged' ? 'warning' : 'success'" size="small">
        ...
        <el-table-column label="操作" width="160" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.status === 'unack'" size="small" @click="ackAlert(row.id).then(fetch)">确认</el-button>
